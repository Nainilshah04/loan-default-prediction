...
# Chart 1: Credit History Impact (MOST IMPORTANT)
...
parts = ax9.violinplot([rejected, approved], positions=[0, 1], showmeans=True, showmedians=True)
for pc in parts['bodies']:
    pc.set_facecolor('#3498DB')
    pc.set_alpha(0.6)
ax9.set_xticks([0, 1])
ax9.set_xticklabels(['Rejected', 'Approved'], fontsize=10)
ax9.set_ylabel('Total Income', fontsize=11, fontweight='bold')
ax9.set_title('Income Distribution', fontsize=13, fontweight='bold', pad=10, color='#2C3E50')
...
ax10.set_xticklabels(['Graduate', 'Not Graduate'], fontsize=10)
ax10.set_ylabel('Avg Loan Amount', fontsize=11, fontweight='bold')
ax10.set_title('Loan Amount by Education', fontsize=13, fontweight='bold', pad=10, color='#2C3E50')
...
bars = ax11.bar(['Salaried', 'Self-Employed'], [emp_approval['No'], emp_approval['Yes']], 
                color=['#16A085', '#D68910'], edgecolor='black', linewidth=1.5, width=0.6)
ax11.set_ylabel('Approval Rate (%)', fontsize=11, fontweight='bold')
ax11.set_title('Employment Type Impact', fontsize=13, fontweight='bold', pad=10, color='#2C3E50')
ax11.set_ylim(0, 100)
...
bars = ax12.bar(['Unmarried', 'Married'], [marital_approval['No'], marital_approval['Yes']], 
...
print(f"\n✅ Clean dashboard saved: {os.path.abspath(dashboard_path)}")
